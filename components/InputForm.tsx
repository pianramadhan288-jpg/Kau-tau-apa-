...
const InputForm: React.FC<InputFormProps> = ({ onSubmit, loading, defaultConfig }) => {
  ...
  const renderStep2 = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
          {/* Mode Selector */}
          <div className="flex p-1 bg-[#1a1a1a] rounded-xl border border-[#333] w-fit">
              <button 
                type="button"
                onClick={() => setFundMode('MANUAL')}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${fundMode === 'MANUAL' ? 'bg-white text-black' : 'text-slate-500 hover:text-slate-300'}`}
              >
                  Manual Input
              </button>
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${fundMode === 'PHOTO' ? 'bg-white text-black' : 'text-slate-500 hover:text-slate-300'}`}
              >
                  Photo / Screenshot
              </button>
          </div>
          ...
          {fundMode === 'PHOTO' ? (
              <div className="space-y-6">
                  <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-[#333] rounded-3xl bg-[#111] hover:border-indigo-500/50 transition-colors group">
                      {isExtracting ? (
                          <div className="flex flex-col items-center">
                              <Loader2 size={48} className="text-indigo-500 animate-spin mb-4" />
                              <p className="text-white font-bold">AI is reading {pendingImages.length} image(s)...</p>
                              <p className="text-slate-500 text-xs mt-1">Merging data points into a single report</p>
                          </div>
                      ) : (
                          <>
                              <div className="p-4 bg-[#1a1a1a] rounded-full text-slate-500 mb-4 group-hover:text-indigo-400 transition-colors">
                                  <Camera size={48} />
                              </div>
                              <h3 className="text-lg font-bold text-white mb-2">Upload KeyStats Screenshots</h3>
                              ...
                              <label className="px-8 py-3 bg-white text-black font-bold rounded-xl cursor-pointer hover:bg-slate-200 transition-colors flex items-center gap-2">
              ...
              <>
                  ...
                  <div className="mt-8 pt-8 border-t border-[#222]">
                      <div className="flex items-center gap-2 mb-4">
                          ...
                          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">AI Extracted Fundamentals (From Photos)</label>
                      </div>
                      <textarea 
                        className="w-full bg-[#1a1a1a] border border-[#333] rounded-xl p-4 text-slate-300 font-mono text-xs outline-none h-48 resize-none focus:border-indigo-500" 
                      ...
                      />
                      <p className="text-[10px] text-slate-600 mt-2 italic">*Jika input manual di atas kosong, AI akan menggunakan data dari kotak ini untuk analisis.</p>
                  </div>
              </>
          )}
      </div>
  ...
  const renderStep4 = () => (
      <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
          ...
          <div className="space-y-4">
              <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-widest">Raw Intelligence Data</label>
                  <textarea className="w-full bg-[#1a1a1a] border border-[#333] rounded-xl p-4 text-slate-300 font-mono text-xs outline-none h-48 resize-none focus:border-indigo-500" placeholder="Paste full report..." value={formData.rawIntelligenceData} onChange={(e) => handleChange(null, 'rawIntelligenceData', e.target.value)} />
              </div>
              <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-widest">Corporate Action & News (Optional)</label>
                  <textarea className="w-full bg-[#1a1a1a] border border-[#333] rounded-xl p-4 text-slate-300 font-mono text-xs outline-none h-32 resize-none focus:border-indigo-500" placeholder="Paste news or corporate action info here..." value={formData.newsData} onChange={(e) => handleChange(null, 'newsData', e.target.value)} />
              </div>
          </div>
      </div>
  );
